...
use BlackCat\Database\Packages\RegisterEvents\Dto\RegisterEventDto as Dto;
use BlackCat\Database\Packages\RegisterEvents\Mapper\RegisterEventDtoMapper as RowMapper;
...
final class RegisterEventRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\RegisterEvents\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\RegisterEvents\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RegisterEvents\Dto\RegisterEventDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\RegisterEvents\Dto\RegisterEventDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\RegisterEvents\Mapper\RegisterEventDtoMapper::fromRow($row);
    }
