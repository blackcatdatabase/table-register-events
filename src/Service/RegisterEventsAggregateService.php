 ...
 */
final class RegisterEventsAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RegisterEventRepository $registerEventRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'register_events' => $this->registerEventRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\RegisterEvents\ModuleException("Record {$idStr} not found for locking.");
            }
